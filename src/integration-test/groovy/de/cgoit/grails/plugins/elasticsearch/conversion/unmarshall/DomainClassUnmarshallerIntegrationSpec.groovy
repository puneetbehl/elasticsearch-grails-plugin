...
class DomainClassUnmarshallerIntegrationSpec extends Specification implements de.cgoit.grails.plugins.elasticsearch.ElasticSearchSpec {
    ...
    void 'An unmarshalled geo_point is marshalled into a GeoPoint domain object'() {
        ...
        SearchHit hit = new SearchHit(1, '1', new Text('building'), [:])
                .sourceRef(new BytesArray(
                        '{"location":{"class":"test.GeoPoint","id":"2", "lat":53.0,"lon":10.0},"name":"WatchTower"}'))
        hit.index = 'test.building'
        SearchHit[] hits = [hit]
    ...
    void 'An unmarshalled temporal type is marshalled into a corresponding object'() {
        ...
        SearchHit hit = new SearchHit(1, '1', new Text('dates'), [:])
                .sourceRef(new BytesArray(
                        '{"date":"2019-08-12T07:25:17.935Z","localDateTime":"2019-08-12T09:25:17.935Z","zonedDateTime":"2019-08-12T03:25:17.935-04:00","offsetTime":"03:25:17.935-04:00","name":"Object with java.util.time types","offsetDateTime":"2019-08-12T03:25:17.935-04:00","localDate":"2019-08-12"}'))
        hit.index = 'test.dates'
        SearchHit[] hits = [hit]
        def maxScore = 0.1534264087677002f
        def totalHits = 1
        def searchHits = new SearchHits(hits, new TotalHits(totalHits, TotalHits.Relation.EQUAL_TO), maxScore)
    ...
    void 'An unmarshalled type with configured converter is marshalled into a corresponding object'() {
        ...
        results[0].status != null &&
                results[0].status instanceof Converter.Status && results[0].status == Converter.Status.STATUS1
    }
    ...
    void 'An unhandled property in an embedded indexed domain is ignored'() {
        ...
        SearchHit hit = new SearchHit(1, '1', new Text('circle'), [:])
                .sourceRef(new BytesArray(
                        '{"radius":7, "color":{"class":"test.Color", "id":"2", "name":"Orange", "red":255}}'))
        hit.index = 'test.circle'
        SearchHit[] hits = [hit]
        def maxScore = 0.1534264087677002f
        def totalHits = 1
        def searchHits = new SearchHits(hits, new TotalHits(totalHits, TotalHits.Relation.EQUAL_TO), maxScore)
    ...
    private void withMockLogger(Closure closure) {
        ...
        Logger mockLog = Mock(Logger) {
            ...
            error(_ as String,
                    _ as Throwable) >> { String s, Throwable t -> System.err.println("ERROR: $s"); t.printStackTrace() }
        }
        try {
